...
if(isset($_POST['update'])){
   ...
   $update_listing = $conn->prepare("UPDATE `property` SET property_name = ?, address = ?, price = ?, type = ?, offer = ?, status = ?, furnished = ?, bhk = ?, bathroom = ?, age = ?,  loan = ?, garden = ?, water_supply = ?, power_backup = ?, parking_area = ?, description = ? WHERE id = ?");   
   $update_listing->execute([$property_name, $address, $price, $type, $offer, $status, $furnished, $bhk, $bathroom,  $age,   $loan,   $garden, $water_supply, $power_backup, $parking_area, $description, $update_id]);
